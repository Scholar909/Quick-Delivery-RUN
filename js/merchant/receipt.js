// receipt.js
// Live-updating receipt viewer for merchant
...
function renderReceipt(data, idFromUrl) {
  ...
  let statusField = (data.orderStatus || data.status || '').toLowerCase();
  
  // Merchant-specific status override
  const userId = auth.currentUser?.uid;
  if (userId) {
    // If this merchant declined
    if (data.declines && data.declines[userId]) {
      statusField = "declined";
    }
    // If this merchant is the currently assigned one and status is accepted
    else if (data.assignedMerchantId === userId && data.orderStatus === "accepted") {
      statusField = "pending"; // still working on it
    }
    // If this merchant is the currently assigned one and order delivered
    else if (data.assignedMerchantId === userId && data.orderStatus === "delivered") {
